...
class SiglentSDG1000X:
	def __init__(self):
		...
		self.MemoryFile = "memory.txt"
		self.channels = "2"
		self.nickname = "rnnishiPI0w"
		self.interface = "USB"
		self.FirstOutFile = "FG-Run_"
	...
	def Start(self):
		...
		self.LogAction(key)
		if self.sig == False:
			self.OF.write("Failed to connect. Exiting...")
			self.OFONLINE.write("failed to connect. Exiting...")
			self.CloseMemory()
			sys.exit()
	def Done(self):
		key = "DONE"
